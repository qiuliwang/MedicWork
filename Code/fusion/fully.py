...
class modelfully(object):
    ...
    def __init__(self,learning_rate,keep_prob,batch_size,epoch):
        ...
        with self.graph.as_default():
            ...
            self.net_out = self.archi_1(self.x_image, self.sphericity, self.margin, self.lobulation, self.spiculation, self.keep_prob)
            ...
            self.saver = tf.train.import_meta_graph('./fully_ckpt/fully-120.meta')  # default to save all variable,save mode or restore from path

            self.sess = tf.Session(graph = self.graph)
            # self.sess.run(tf.global_variables_initializer())
            self.saver.restore(self.sess, tf.train.latest_checkpoint('./fully_ckpt/'))
            self.sess.run(tf.global_variables_initializer())
