...
class modelsphercity(object):
    ...
    def __init__(self,learning_rate,keep_prob,batch_size,epoch):
        ...
        with self.graph.as_default():
            ...
            self.net_out = self.archi_sphercity(self.x_image, self.sphericity, self.margin, self.lobulation, self.spiculation, self.keep_prob)
            ...
            self.saver = tf.train.import_meta_graph('./sphercity_ckpt/fully-80.meta')  # default to save all variable,save mode or restore from path

            self.sess = tf.Session(graph = self.graph)
            # self.sess.run(tf.global_variables_initializer())
            self.saver.restore(self.sess, tf.train.latest_checkpoint('./sphercity_ckpt/'))
            self.sess.run(tf.global_variables_initializer())
    ...
    def archi_sphercity(self,input,sphericity, margin, lobulation, spiculation, keep_prob):
        ...
        # return out_fc2
        with tf.name_scope("Archi-Sphercity"):
            # input size is batch_sizex20x20x6
            # 5x5x3 is the kernel size of conv1,1 is the input depth,64 is the number output channel
            w_conv1 = tf.Variable(tf.random_normal([3, 5, 5, 1, 64],stddev=0.01),dtype=tf.float32,name='w_conv1')
            b_conv1 = tf.Variable(tf.constant(0.01,shape=[64]),dtype=tf.float32,name='b_conv1')
