...
class modelfpr(object):
    ...
    def __init__(self,learning_rate,keep_prob,batch_size,epoch):
        ...
        self.saver = tf.train.import_meta_graph('/home/wangqiuli/Documents/august_fpr/fully_ckpt/fully-40.meta')  # default to save all variable,save mode or restore from path

        # self.sess.run(tf.global_variables_initializer())
        self.saver.restore(self.sess, tf.train.latest_checkpoint('/home/wangqiuli/Documents/august_fpr/fully_ckpt/'))
    ...
    def pred(self, test_batch,  test_label):
        dropkeep = 1
        test_dict = {self.x: test_batch,self.real_label:test_label, self.keep_prob:dropkeep}

        predres, aacres = self.sess.run([self.prediction, self.accruacy], feed_dict = test_dict)
        # countx = 0
        # for one in test_label:
        #     if one[0] == 1:
        #         countx += 1
        # print(countx / len(test_batch))
        # print(aacres)
