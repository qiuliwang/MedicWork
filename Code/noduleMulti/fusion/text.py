...
class modeltext(object):
    ...
    def __init__(self,learning_rate,keep_prob,batch_size,epoch):
        ...
        self.saver = tf.train.import_meta_graph('../august_text/fully_ckpt/fully-80.meta')  # default to save all variable,save mode or restore from path

        # self.sess.run(tf.global_variables_initializer())
        self.saver.restore(self.sess, tf.train.latest_checkpoint('../august_text/fully_ckpt/'))
